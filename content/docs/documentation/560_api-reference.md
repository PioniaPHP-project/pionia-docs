...
{{< callout tip >}}
**Prerequisites:** You have a [service](/documentation/services/services/) with `*Action` methods registered on a [switch](/moonlight/api-versioning-in-moonlight/). This guide covers **documenting** those actions for humans and API consumers — not writing the business logic itself.
{{< /callout >}}
...
| `@moonlight-table` | Optional | Primary table for [generic CRUD](/documentation/services/generic-services/) actions. |
...
Every action returns the same JSON shape (see [Actions](/documentation/services/actions/)):
...
See also [Developer stats](/documentation/developer-stats/) — `/stats` uses a separate `STATS_TOKEN`.
...
- [Actions](/documentation/services/actions/) — write action methods and envelopes
- [Security](/documentation/security/security-authentication-and-authorization/) — auth traits and `@moonlight-auth`
- [CLI commands](/documentation/commands-pionia-cli/) — full `api:docs` / `api:catalog` reference
- [Moonlight architecture](/moonlight/introduction-to-moonlight-architecture/) — switches, services, and the request model

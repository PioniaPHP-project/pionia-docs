...
{{< callout note >}}
`Arrayable` wraps JSON and form fields with typed getters (`getString`, `getInt`, …). See [Collections (Arrayable)](/documentation/http/collections/) and [Actions](/documentation/building-api/actions/) for the full accessor reference.
{{</ callout >}}
...
For how routes are matched and cached in production, see [HTTP routing](/documentation/http/http-routing/).
{{</callout>}}
...
See [Actions — request files](/documentation/building-api/actions/#request-files).
...
See [Validations](/documentation/building-api/validation/) for `#[ValidateField]`, custom rules, and the full rule list.
...
See [Exceptions & error handling](/documentation/http/exceptions/) for status codes and the pipeline.
...
SPA client routes (e.g. `/dashboard`) fall back to `index.html` when configured — see [Frontend integration](/documentation/frontend/vite-integration/).
